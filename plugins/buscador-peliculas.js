...
const handler = async (m, {text, usedPrefix, command, conn}) => {
  if (!text) throw '*[❗] أدخل اسم الفيلم للبحث*';
  let aaaa;
  let img;
  try {
  ...
  if (aaaa == '') throw '*[❗]لم يتم العثور على أي فيلم*';
  const res = await aaaa.map((v) => `*🎬 • اسم:* ${v.title}\n*🍿 • Url:* ${v.link}`).join`\n\n───────────────\n\n`;
  const ads = '*💫 • مانع الإعلانات الموصى به:* منع هذا\n*⛨ •الرالط:* https://block-this.com/block-this-latest.apk\n\n≣≣≣≣≣≣≣≣≣≣≣≣≣≣≣≣≣≣≣≣≣≣≣≣≣≣≣≣≣≣≣≣\n\n';
...
const handler = async (m, { text, usedPrefix, command, conn }) => {
  if (!text) throw '*[❗] اسم الفيلم أو المسلسل مفقود*'

  let result = await searchContent(text)
  if (command === 'pelisplus') result = await searchPelisPlus(text)

  if (result.length === 0) throw '*[❗] لم يتم العثور على محتوى ذي صلة*'
  ...
  const res = result.map((v) => `*🎬 • اسم:* ${v.title}\n*🍿 • URL:* ${v.link}`).join(`\n\n───────────────\n\n`)
