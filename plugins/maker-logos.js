...
const handler = async (m, {conn, args: [effect], text: txt, usedPrefix, command, name}) => {
  if (!effect) throw '*[❗𝐈𝐍𝐅𝐎❗] كيفية استخدام هذا الأمر؟*\n—◉ _#logo (efecto) (texto)_\n*مثال:*\n—◉ _#logo 3d-deep-sea-metal Mystic_\n\n*[❗] عندما أخبرهم أن هناك حاجة إلى نص، فإن 𝚄𝚂𝙾 سيكون كذلك:*\n—◉ _#logo (efecto) (texto1|texto2)_\n*مثال:*\n—◉ _#logo Wolf-Logo-Galaxy Mystic|Bot_\n\n*<قائمة التأثيرات/>*\n\n° ඬ⃟📝 #logo ' + effects.map((v) => v.title).join('\n° ඬ⃟📝 #logo ');
  if (!effects.find((v) => (new RegExp(v.title, 'gi')).test(effect))) throw `*[❗𝐈𝐍𝐅𝐎❗] التأثير ${effect} وهو ليس في قائمة التأثيرات*`;  
