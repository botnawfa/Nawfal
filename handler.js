...
const messageText = `_*< مستخدم معلق />*_\n
 ▢ *يلاحظ:* ${messageNumber}/3
 ${user.bannedReason ? `\n▢ *سبب:* ${user.bannedReason}` : ' ▢ *سبب:* غير محدد'}
 *[ ℹ️ ] إذا كنت تعتقد أن هذا خطأ ولديك دليل، فيمكنك الاتصال بمالك الروبوت لاستئناف التعليق.*`.trim();
              m.reply(messageText);
        ...
        try {
          ...
          if (m.limit) {
            m.reply('*[ ℹ️ ] تم استخدامها ' + +m.limit + ' الماس (الماس) (الحدود).*');
          }
        }
        break;
      }
  ...
  for (const nk of callUpdate) {
    if (nk.isGroup == false) {
      if (nk.status == 'offer') {
        ...
        const vcard = `BEGIN:VCARD\nVERSION:3.0\nN:;Nawfal nl 👑;;;\nFN:Nawfal Nl 👑\nORG:Nawfal Nl 👑\nTITLE:\nitem1.TEL;waid=212707676259:+212 707 676 259\nitem1.X-ABLabel:Nawfal Nl  👑\nX-WA-BIZ-DESCRIPTION:[❗] ᴄᴏɴᴛᴀᴄᴛᴀ ᴀ ᴇsᴛᴇ ɴᴜᴍ ᴘᴀʀᴀ ᴄᴏsᴀs ɪᴍᴘᴏʀᴛᴀɴᴛᴇs.\nX-WA-BIZ-NAME:𝐁𝐫𝐮𝐧𝐨 𝐒𝐨𝐛𝐫𝐢𝐧𝐨 👑\nEND:VCARD`;
        await mconn.conn.sendMessage(nk.from, {contacts: {displayName: 'Nawfal Nl 👑', contacts: [{vcard}]}}, {quoted: callmsg});
...
global.dfail = (type, m, conn) => {
  ...
  const prep = generateWAMessageFromContent(m.chat, {extendedTextMessage: {text: msg, contextInfo: {externalAdReply: {title: '*[ ⚠ ] Advertencia*', body: 'ᴛʜᴇ ᴍʏsᴛɪᴄ - ʙᴏᴛ', thumbnail: imagen1, sourceUrl: 'https://instagram.com/nawfal_the_ghost'}}}}, aa);
